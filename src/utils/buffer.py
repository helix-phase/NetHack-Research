...
class StateOnlySequentialBuffer:
    """
    A simplified sequential buffer that works with state-only datasets
    """
    ...
    def __init__(
        ...
        # We don't need to pre-compute actions anymore since we've simplified the dataset to use zeros
        print(f"Sequential buffer initialized with batch size {batch_size} and sequence length {seq_len}")
